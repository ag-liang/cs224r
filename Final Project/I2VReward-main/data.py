...
def convert_GSB_csv_to_reward_data(example, data_dir, eval_dims=["VQ"], max_pixels=448 * 448, fps=2.0, 
    ...
    A_data = [
    {
        ...
        "content": [
            {
                "type": "image",
                "image": f"file://{data_dir}/{example['source_image']}"
            },
            {
                "type": "video", 
                ...
                "nframes": min(num_frames, example.get("num_frames_A", 16)) if num_frames is not None else None,
    ...
    B_data = [
        {
            ...
            "content": [
                ...
                },
                {
                    ...
                    "max_pixels": max_pixels, 
                    "fps": fps if num_frames is None else None,
                    "nframes": min(num_frames, example.get("num_frames_B", 16)) if num_frames is not None else None,
                    "sample_type": sample_type,
                }
            ]
        }
        ...
        out_message = [
            {
                ...
                "content": [
                    {
                        "type": "image",
                        "image": message[0]["content"][0]["image"]
                    },
                    {
                        "type": "video", 
                        "video": message[0]["content"][1]["video"], 
                        "max_pixels": message[0]["content"][1]["max_pixels"], 
                        "fps": message[0]["content"][1]["fps"] if "fps" in message[0]["content"][1] else None,
                        "nframes": message[0]["content"][1]["nframes"] if "nframes" in message[0]["content"][1] else None,
                        "sample_type": message[0]["content"][1]["sample_type"] if "sample_type" in message[0]["content"][1] else "uniform",
                    }
                ],
            }
        ]

        if out_message[0]["content"][1]["fps"] is None:
            out_message[0]["content"][1].pop("fps")
        if out_message[0]["content"][1]["nframes"] is None:
            out_message[0]["content"][1].pop("nframes")
