...
def update_replay_buffer(
    ...
    for timestep in range(len(episode_experience)):
        ...
        if her_type == HERType.FINAL:
            ...
            final_state = episode_experience[-1][3].copy()  # Get final state
            
            # compute new reward
            new_reward = env_reward_function(final_state, goal)
            ...
            for _ in range(num_relabeled):
                ...
                future_idx = np.random.randint(timestep, len(episode_experience))
                future_state = episode_experience[future_idx][3].copy()
                
                # compute new reward
                new_reward = env_reward_function(future_state, goal)
                
                # add to replay buffer
                replay_buffer.add(np.append(state, future_state),
                                action,
                                new_reward,
                                np.append(next_state, future_state))
            ...
            for _ in range(num_relabeled):
                # get random goal
                random_idx = np.random.randint(0, len(episode_experience))
                random_state = episode_experience[random_idx][3].copy()
                
                # compute new reward
                new_reward = env_reward_function(random_state, goal)
                
                # add to replay buffer
                replay_buffer.add(np.append(state, random_state),
                                action,
                                new_reward,
                                np.append(next_state, random_state))
