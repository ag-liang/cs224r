...
class EncoderDecoder(Embedder, relabel.RewardLabeler):
    ...
    def _compute_losses(
        ...
        # Detach id_embeddings to stop gradient flow
        detached_id_embeddings = id_embeddings.detach()
        
        # Expand id_embeddings to match the shape of all_decoder_embeddings
        # Shape: (batch_size, 1, embed_dim)
        expanded_id_embeddings = detached_id_embeddings.unsqueeze(1)
        
        # Compute squared L2 norm between decoder embeddings and id embeddings
        # Shape: (batch_size, episode_len + 1)
        decoder_context_loss = torch.sum(
            (all_decoder_embeddings - expanded_id_embeddings) ** 2,
            dim=-1
        )
    ...
    def label_rewards(self, trajectories):
        ...
        # Compute rewards as the difference in log probabilities
        # Shape: (batch_size, episode_len)
