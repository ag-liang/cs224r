...
class IQLCritic(BaseCritic):
    ...
    def __init__(self, hparams, optimizer_spec, **kwargs):
        ...
        self.v_net = network_initializer(self.ob_dim, 1)  # Value function outputs a single value
        ### YOUR CODE END HERE ###
    ...
    def update_v(self, ob_no, ac_na):
        ...
        ### YOUR CODE START HERE ###
        q_values = self.q_net_target(ob_no)
        q_values = q_values.gather(1, ac_na.unsqueeze(1))
        v_values = self.v_net(ob_no)
    ...
    def update_q(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        ### YOUR CODE START HERE ###
        q_values = self.q_net(ob_no)
        q_values = q_values.gather(1, ac_na.unsqueeze(1))
        
        with torch.no_grad():
            next_v_values = self.v_net(next_ob_no)
            targets = reward_n.unsqueeze(1) + self.gamma * next_v_values * (1 - terminal_n.unsqueeze(1))
