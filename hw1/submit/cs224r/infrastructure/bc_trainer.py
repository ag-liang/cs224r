...
class BCTrainer:
    ...
    def collect_training_trajectories(
        ...
        if itr == 0 and load_initial_expertdata is not None:
            try:
                ...
                print("Could not load expert data. Collecting new trajectories instead.")
        ...
        if self.log_video:            
            ...
            # Use sample_trajectory directly to ensure proper video recording
            train_video_paths = []
            for _ in range(MAX_NVIDEO):
                path = utils.sample_trajectory(self.env, collect_policy, MAX_VIDEO_LEN, True)
                train_video_paths.append(path)
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for path in paths:
            ...
            # Get expert actions for each observation
            for obs in observations:
                # Convert tensor to numpy first, then back to tensor
                if isinstance(obs, torch.Tensor):
                    obs = obs.cpu().numpy()
                action = expert_policy.get_action(obs)
                # Squeeze any extra dimensions to match expected shape
                action = np.squeeze(action)
                expert_actions.append(action)
                
            # Replace original actions with expert actions
            path['action'] = np.array(expert_actions)
            # Ensure actions have correct shape (T, action_dim)
            if len(path['action'].shape) == 3:
                path['action'] = np.squeeze(path['action'], axis=1)
