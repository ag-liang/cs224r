...
class ACAgent:
    ...
    def update_critic(self, replay_iter):
        ...
        with torch.no_grad():
            ...
            next_action = next_action_dist.sample(clip=None)
        ...
        #critic loss
        critic_loss = sum(F.mse_loss(current_q, target.detach()) for current_q in current_qs)
        ...
        with torch.no_grad():
            for param, target_param in zip(self.critic.parameters(), self.critic_target.parameters()):
                target_param.data.copy_(
                    self.critic_target_tau * param.data + (1 - self.critic_target_tau) * target_param.data
                )
    ...
    def update_actor(self, replay_iter):
        ...
        q = torch.min(torch.stack(qs), dim=0)[0]
    ...
    def bc(self, replay_iter):
        ...
        #calculate the MSE loss between predicted and expert actions
