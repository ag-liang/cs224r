...
def update_replay_buffer(
    ...
    for timestep in range(len(episode_experience)):
        ...
        if her_type == HERType.FINAL:
            ...
            final_goal = episode_experience[-1][3].copy()  # next_state of last transition
            new_goal = final_goal
            new_reward = env_reward_function(next_state, new_goal)
            ...
            for _ in range(num_relabeled):
                if timestep + 1 >= len(episode_experience):
                    break  # no future steps
                future_idx = np.random.randint(timestep + 1, len(episode_experience))
                future_goal = episode_experience[future_idx][3].copy()  # next_state at future_idx
            ...
            for _ in range(num_relabeled):
                rand_idx = np.random.randint(0, len(episode_experience))
                rand_goal = episode_experience[rand_idx][3].copy()  # next_state at rand_idx
                new_reward = env_reward_function(next_state, rand_goal)
                replay_buffer.add(np.append(state, rand_goal),
                                  action,
                                  new_reward,
                                  np.append(next_state, rand_goal))
